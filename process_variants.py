...
def getCallsetFolder(patientid, kind = 'original'):
	""" 
		...
		----------
			...
			chrom: string
				If provided, the callset corresponding to 'kind' will
				be split by chromosome and the folder corresponding
				to the selected callset will be returned.
...
def getPipelineFolder(step, **kwargs):
	...
	if 'callsets' in step:
		...
		if 'patientId' not in kwargs and 'sample' not in kwargs:
			message = "Pipeline step '{}' requires one of ('patientId' or 'sample')".format(step)
			message += "\n supplied " + ", ".join(kwargs.keys())
			raise ValueError(message)
	...
	elif step == 'somaticseq-training-tables':
		folder_names = ('somaticseq', 'training', 'individual_tables', kwargs['patientId'])
	elif step == 'somaticseq-merged':
		folder_names = ('somaticseq', 'merged_tables')
	elif step == 'somaticseq-classifier':
		folder_names = ('somaticseq', 'classifier')
	elif step == 'somaticseq-prediction':
		folder_names = ('somaticseq', 'prediction', kwargs.get('kind'))
	elif step == 'somaticseq-prediction-tables':
		folder_names = ('somaticseq', 'prediction', 'tables', kwargs['patientId'])
...
class SomaticSeqPipeline:
		...
		"""
			...
			-------
				training_result: dict<>
					* 'untrainedIndels': string
					* 'untrainedSnps': string
					* 'indelTable': string
					* 'snpTable': string
	...
	def _combineTables(self, tables):
		...
		snp_table_filenames = [t['snpTable'] for t in tables]
		indel_tables = list()
		snp_tables = list()

		if len(tables) > 0:
			for indel_filename in indel_table_filenames:
				indel_table, indel_fieldnames = tabletools.readCSV(indel_filename, True)
				indel_tables += indel_table
			
			# Need to sort the table.
			print("Should probably sort the combined table.")
			for snp_filename in snp_table_filenames:
				snp_table, snp_fieldnames = tabletools.readCSV(snp_filename, True)
				snp_tables += snp_table
		else:
			message = "In _combineTables(), no tables were provided."
			raise ValueError(message)

		tabletools.writeCSV(indel_tables, merged_indel_table_filename, fieldnames = indel_fieldnames)
		tabletools.writeCSV(snp_tables,     merged_snp_table_filename, fieldnames = snp_fieldnames)

		output = {
			'snpTable':   merged_snp_table_filename,
			'indelTable': merged_indel_table_filename
		}
		return output
	...
	def _runManualClassifier(self, indel_table, snp_table):
		...
		indel_command = command.format(
			script = self.ada_builder_script, 
			table = indel_table
		)
		snp_command = command.format(
			script = self.ada_builder_script, 
			table = snp_table)

		expected_output = {
			'indelTable':     os.path.join(output_folder, "Ensemble.sINDEL.tsv"),
			'snpTable':       os.path.join(output_folder, "Ensemble.sSNV.tsv"),
			'indelClassifier': os.path.join(output_folder, "Ensemble.sINDEL.Classifier.RData"),
			'snpClassifier':  os.path.join(output_folder, "Ensemble.sSNV.Classifier.RData")
		}
	...
	def _runPredictor(self, patient_info, classifier):
		...
		command = """{somaticseq} \
			...
			--output-dir {output_folder}""".format(
				...
				snv_classifier   = classifier['snpClassifier'],
				indel_classifier = classifier['indelClassifier'],
				output_folder    = prediction_output_folder)

		indel_table = os.path.join(prediction_output_folder, "Ensemble.sINDEL.tsv")
		snp_table   = os.path.join(prediction_output_folder, "Ensemble.sSNV.tsv")
		...
		expected_output = {
			...
			'indelVcf':   indel_vcf,
			'snpVcv':     snp_vcf,
			'indelClassifer': os.path.join(prediction_output_folder),
			'snvClassifier':  os.path.join(prediction_output_folder)
		}
